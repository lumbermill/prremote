...
- Example (**M5StickC PLUS**, `examples/m5stickc_plus/lcd_hello.rb`, `ntp_clock.rb`, `fablab.rb`): red/blue were swapped on this ST7789 panel — text drawn with `LCD::RED` rendered blue on physical hardware. The default MADCTL table's BGR bit (`0x08`) is tuned for the M5GO's ILI9342C; this panel wants plain RGB. All three now pass `madctl:` explicitly (`0x00` at rotation 0, `0xA0` at rotation 3 — the rotation-3 table entry `0xA8` minus the BGR bit) via the existing `LCD.new(madctl:)` override, no runtime change needed. _Verified on a physical M5StickC PLUS: red/green/blue test text and each script's actual colors (white/cyan/orange in `lcd_hello.rb`, the red `Auth failed` error screen in `ntp_clock.rb`, and the full color list including blue in `fablab.rb`) all render correctly._
...
- Example: `examples/m5stickc_plus/buttons.rb` — reads both buttons: GPIO37 (A, front) and GPIO39 (B, side, opposite the power button). Both input-only pins with an on-board external pull-up; pressed reads 0. Button B needed a second, more careful probing pass to confirm: GPIO35 (a candidate from the first pass) turned out to read a constant floating LOW unrelated to any button, and a single-pin retest without GPIO35 alongside it had bad luck with polling timing. Multi-pin probing with only-log-on-change caught GPIO39 toggling in step with the button — GPIO39 matches the M5Stack Arduino library's `Config.h` after all. _Both verified on a physical M5StickC PLUS._
...
- Example: `examples/m5stickc_plus/fablab.rb` — joke sample: "Fablab" filling the landscape-rotated screen, button A blinks the text to a new color and button B blinks the background instead (handy as a visual "which button did I just press?" check). First real-hardware exercise of the `LCD.new(rotation:)` odd-rotation path added alongside `width:`/`height:`/`offset_x:`/`offset_y:` above — `LCD#width`/`#height` correctly report 240x135 swapped. `rotation: 3` is what "90° clockwise" turned out to mean on this panel (`rotation: 1` read upside down) — determined empirically since the MADCTL table was tuned for the ILI9342C, not this ST7789. _Verified on a physical M5StickC PLUS: correct orientation, and both buttons reliably blink their respective color list._
- Example: `examples/m5stickc_plus/ruby.rb` — `fablab.rb` variant: "Ruby" with a small hand-drawn gem icon (flat-top faceted diamond, same shape idea as the M5GO `ntp_clock.rb` logo, halved) in front of the text, initial color red. Same button A/B color-blink behavior. _Verified on a physical M5StickC PLUS: icon and text render correctly-proportioned and centered, with the post-MADCTL-fix correct red._
