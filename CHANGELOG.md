...
- Example: `examples/m5stickc_plus/fablab.rb` — joke sample: "Fablab" filling the landscape-rotated screen, button A blinks it to a new color. First real-hardware exercise of the `LCD.new(rotation:)` odd-rotation path added alongside `width:`/`height:`/`offset_x:`/`offset_y:` above — `LCD#width`/`#height` correctly report 240x135 swapped. `rotation: 3` is what "90° clockwise" turned out to mean on this panel (`rotation: 1` read upside down) — determined empirically since the MADCTL table was tuned for the ILI9342C, not this ST7789. _Verified on a physical M5StickC PLUS: correct orientation, and button A reliably blinks through the color list._
