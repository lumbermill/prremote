...
module Prremote
  module Commands
    class Deploy
      ...
      def mrbc_bin
        # MRBC env var allows explicit override
        return ENV["MRBC"] if ENV["MRBC"] && File.executable?(ENV["MRBC"])

        # Search PATH excluding rbenv shims, which intercept mrbc and report
        # "command not found" when the active Ruby version is not mruby.
