/*
 * TCPSocket VM bindings (mruby/c) — vendored from picoruby with refcount fixes.
 *
 * Origin: runtime/picoruby/mrbgems/picoruby-socket/src/mrubyc/tcp_socket.c
 * (submodule @ 761e2cf6). The upstream file leaks references:
 *
 *  - Every method except new/ready? (send, readpartial, read_nonblock,
 *    close, closed?, remote_host, remote_port — 7 sites) calls
 *    mrbc_incref(&v[0]) right before SET_*_RETURN().
 ...
 *    Same receiver over-incref as udp_socket.c; see udp_socket_binding.c.
 *
 * The string-returning paths (readpartial, read_nonblock, remote_host) were
 * audited for the other udp_socket.c bug (extra incref on a freshly created
 * return value): upstream already stores the new string via SET_RETURN with
 * no extra incref, so those paths are unchanged.
 *
