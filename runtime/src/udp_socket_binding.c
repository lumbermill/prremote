/*
 * UDPSocket VM bindings (mruby/c) — vendored from picoruby with refcount fixes.
 *
 * Origin: runtime/picoruby/mrbgems/picoruby-socket/src/mrubyc/udp_socket.c
 * (submodule @ 761e2cf6). The upstream file leaks references:
 *
 *  - c_udp_socket_recvfrom_nonblock over-increfs every element of the
 *    returned [data, addr_info] structure. mrbc_array_set() takes ownership
 *    of the value (it does not incref internally), so a freshly created
 *    object (ref_count 1) must be stored without an extra incref. The
 *    upstream extra increfs leak the whole result structure on every
 *    successful receive — "Fatal error: Out of memory." after ~200 calls.
 *
 *  - Every method calls mrbc_incref(&v[0]) right before SET_*_RETURN().
 *    SET_*_RETURN() releases the receiver's register reference exactly once
 *    (see SET_RETURN in mrubyc/src/value.h; the VM only decrefs argument
 *    registers after a C method, vm.c OP_SEND). The extra incref therefore
 *    adds +1 to the socket's 16-bit ref_count per call until it saturates
 *    at 0xFFFF — "assert failed: mrbc_decref value.h:566" after ~65k calls.
 *
 * Everything else is kept identical to upstream so the two files stay easy
 * to diff. Drop this copy and switch the build back to the submodule file
 * once the fixes land upstream.
 */
#include <stdlib.h>
#include <string.h>
#include <stdint.h>
...
c_udp_socket_recvfrom_nonblock(mrbc_vm *vm, mrbc_value *v, int argc)
{
   ...
   * mrbc_array_set() takes ownership of the stored value, so freshly created
   * objects (ref_count 1) are stored as-is with no extra incref. */
  ...
  /* host_val is stored in two slots, so it needs one extra reference. */
